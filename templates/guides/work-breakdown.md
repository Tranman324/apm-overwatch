...
1. Analyze for universal execution patterns across all planning sources:
   ...
   <!-- OVERWATCH BEGIN -->
   - **QA gate rules:** When gathered context shows recurring false-green risk, release-critical execution, template/procedure generation, autonomous dispatch or recovery, Handoff behavior, security/privacy, schemas/APIs, multi-module work, or larger changes, consider compact Overwatch-style QA gate rules. Classify them as Rules only when the validation discipline should apply broadly; otherwise keep validation expectations in Task guidance. Do not paste long process prose - generate self-contained conditional standards that name the lightweight/full gate split, critic responsibilities, narrative skepticism, a required negative control or sabotage for non-trivial work (or why it is impractical and the strongest substitute), tiered reporting, and the coverage boundary.
   <!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Overwatch QA gate rule shape:** When QA gate rules are warranted, keep them self-contained inside APM_RULES. Distinguish trivial, contained work that can use lightweight validation from non-trivial work requiring a full two-critic gate, including generated template procedures, release-critical behavior, Handoff/recovery behavior, autonomous dispatch behavior, security/privacy, schemas/APIs, multi-module work, and larger changes. Define the Test Gate Critic as checking whether proof exercises the work trajectory and was not weakened; define the Adversarial Change Critic as treating reports and narratives as untrusted, checking whether the invariant is structurally closed, and requiring a negative control or sabotage for non-trivial work, or a documented impracticality reason and strongest substitute. Require tiered reporting: compact reports for small work, fuller evidence for cross-boundary, gate-critical, rejected, or autonomous-recovery work. State the coverage boundary directly: execution-layer failures such as stalls, weak reports, fake-green evidence, evidence ambiguity, rejection loops, and abandoned output are in scope; spec/promise drift, product gaps, requirement misreads, and promises dropped from planning documents are not solved.
<!-- OVERWATCH END -->
