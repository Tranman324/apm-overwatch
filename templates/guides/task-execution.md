...
<!-- OVERWATCH BEGIN -->
**THINK / WORK / TRY loop:** Before changing files, THINK: inspect the Task context and current code, name the invariant to close, identify hazards, verify or reproduce the premise before fixing, diagnose the cause before proposing a fix, and map relevant readers or writers of touched state when applicable. For a follow-up that directs a fix for confirmed-defect rejected or Failed work, restate its `Root Cause:` and fix that cause at class level rather than only the cited case; if fresh evidence no longer supports the premise, re-diagnose and report it. WORK: make a narrow coherent change that preserves explicit non-scope and compatibility unless a breaking change is required. TRY: prove the Task trajectory and its front-loaded negative control through the real path when practical, record the commands or artifacts used, and explain in the report how the fix addresses the root cause.
