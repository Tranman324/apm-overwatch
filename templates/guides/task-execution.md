...
<!-- OVERWATCH BEGIN -->
**Scope echo and authorization:** Before editing on every Task, send the Manager a 2-3 sentence echo naming the understood change, expected files or target set, artifacts, and non-scope; wait for execution authorization. When no direct agent channel exists, present the echo to the User as a relay-ready message. If implementation needs an unbriefed file, dependency, infrastructure, artifact, or other out-of-fence work, stop and request adjudication instead of expanding autonomously. For a batch, echo all included Tasks together when requested and begin only those authorized.

**Task-relative proof:** When a validation command can be run, run it and paste the real command output in the Task Log; "tests pass" without command output is insufficient. For code behavior, prefer proof that exercises the real path and would fail under the broken or missing behavior when practical. Demonstrate a negative control only when the Proof Gate names plausible false-green risk. Match proof to what the harness can reliably repeat, preferring deterministic headless/offscreen evidence over interactive, focus-seizing, or credential-prompting runs; do not run screen-seizing or credential-prompting proof without explicit User timing approval. For template-only or documentation work, use the strongest practical artifact proof - static diff evidence, generated-bundle inspection, artifact paths, or simulation - and state why fail-under-broken proof is impractical.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**THINK / WORK / TRY loop:** Before changing files, THINK: inspect the Task context and current code, name the invariant to close, identify hazards, verify or reproduce the premise before fixing, diagnose the cause before proposing a fix, and map relevant readers or writers of touched state when applicable. For a follow-up that directs a fix for confirmed-defect rejected or Failed work, restate its `Root Cause:` and fix that cause at class level rather than only the cited case; if fresh evidence no longer supports the premise, re-diagnose and report it. WORK: make a narrow coherent change that preserves explicit non-scope and compatibility unless a breaking change is required. TRY: prove the Task trajectory, including any named negative control, through the real path when practical; record the commands or artifacts used and explain in the report how the fix addresses the root cause.

**Pattern-class correction:** When the first rejection identifies a pattern class, inventory the Manager-defined Plan target set before editing, correct every in-scope match, and report out-of-scope matches without modifying them. Follow the Correction Envelope; critic suggestions do not authorize expansion.
...
<!-- OVERWATCH BEGIN -->
**Batched preflight:** When the Manager requests it, send one compact Scope Echo covering each Task in the batch and wait for the Manager's batched execution authorization before starting the authorized Tasks.
<!-- OVERWATCH END -->
...
Perform the following actions:
<!-- OVERWATCH BEGIN -->
- **Preflight:** Send the Scope Echo and receive execution authorization before the first edit. A correction or proposed expansion changes the echo and requires confirmation.
<!-- OVERWATCH END -->
1. Execute Detailed Instructions sequentially, applying Guidance and relevant Rules from `{RULES_FILE}`, working toward the Objective. Use the THINK / WORK / TRY loop when planning and proving each meaningful change.
...
<!-- OVERWATCH BEGIN -->
3. If the correction does not resolve the issue, re-examine the premise against fresh evidence before a second fix cycle, separating "the work is wrong" from "my diagnosis is wrong." Then spawn a debug subagent per §2.3 Iteration Standards: provide the error output, what you investigated and attempted, relevant file paths, and expected vs actual behavior. Direct it to trace the root cause and propose a fix.
<!-- OVERWATCH END -->
