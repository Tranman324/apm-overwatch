...
<!-- OVERWATCH BEGIN -->
**Overwatch review validation** - The Manager treats Worker reports and summaries as hypotheses until checked against current artifacts. Review maps claims to diffs, files, generated bundles, command output, Task Logs, commits, runtime evidence, or Tracker state; checks freshness; and prefers evidence one level closer to production than the edited template or source. Trivial, contained work can use lightweight validation. Non-trivial work uses a two-critic gate: the Test Gate Critic checks whether proof exercises the Task trajectory and was not weakened, and the Adversarial Change Critic checks whether the change structurally closes the invariant, treats narratives as untrusted, and includes a negative-control or sabotage consideration when practical. Review reporting states which path was used, the evidence checked, and any residual risk.

**Overwatch review verdicts** - `MERGE_OK`, `MERGED_WITH_RESIDUALS`, `FIX_FIRST`, and `REVERT_OR_ROLL_FORWARD` are review evidence labels, not replacements for Task lifecycle states or Task outcome statuses. When a review rejects work, the Manager attaches critic findings to the follow-up dispatch and states what was rejected and why. If the same Task is rejected twice, the Manager halts dispatch for that Task and escalates with both rejection summaries and a recommendation.
<!-- OVERWATCH END -->
