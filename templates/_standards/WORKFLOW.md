...
**Token-efficient coordination** - Reviews stay within the pinned diff, direct dependency neighborhood, and required production-real artifact or contract references. Re-review is surgical unless the amendment expands that surface. Manager status updates occur only on dispatch, pass, rejection, halt, or escalation; waiting requires a progress signal, reason, and bounded next check.
<!-- OVERWATCH END -->
