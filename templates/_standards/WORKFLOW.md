...
<!-- OVERWATCH BEGIN -->
**Overwatch operating model** - Overwatch preserves APM artifacts and vocabulary while adding execution-layer verification and autonomous-operation discipline. When platform tooling allows, the Manager may create, resume, and poll subagent Workers directly, but the Message Bus, Tracker, Task Logs, Task Reports, branches, worktrees, and artifacts remain the authoritative coordination record. The User remains the approval point for human-only actions, live-state changes, external credentials, money, legal or product risk, and accepting launch risk.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
Overwatch extends the Implementation Phase by tightening proof, review, continuity, and autonomous coordination around the existing Task lifecycle. It catches execution-layer failures such as stalls, weak reports, fake-green evidence, evidence ambiguity, rejection loops, and abandoned Worker output, and applies bounded planning scrutiny to declared invariant layers and shared-state write/reaction coverage. It cannot prevent host-enforced turn termination, disconnection, or forced interruption; recovery resumes coordination from durable artifacts. It does not guarantee spec correctness or solve general spec/promise drift, product gaps, requirement misreads, or planning promises that implementation quietly drops.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Overwatch dispatch packet** - Task Prompts include role, work scope, invariant, non-scope, environment prerequisites, proof gate, output contract, and routing identity when those elements are needed to constrain autonomous Worker execution. Vague "all X" Tasks require inventory before edits begin. The Manager splits work that crosses ownership boundaries, requires a contract change in another function/module, has more than one independent acceptance gate, or cannot fit a single Worker-owned proof gate.

**Autonomous coordination** - At the start of continuous coordination and after Handoff or recovery, the Manager records `DIRECT` in Tracker Working Notes when it can create and poll subagent Workers itself; otherwise it records `RELAY`. `DIRECT` operation overrides command-mediated instructions to end a turn, await User-delivered reports, or ask the User to return the next report. The Manager polls active Workers every 5-10 minutes, or every 2-5 minutes for tiny or near-complete Tasks. Observable progress means a current active child handle, recent Task Log update, file change, running command, Report Bus update, commit, or new concrete blocker. An active-status claim cites one of those signals and is followed by the next bounded poll, wait, review, dispatch, or recovery action; prose about continuing is not a coordination action.

In `DIRECT` operation, the Manager does not end its current turn while a Worker, reviewer, or validation command is active; a report or candidate awaits review; a Ready Task exists; or safe in-scope remediation remains. Local tests, builds, critic reviews, routine merges, and Correction Envelopes continue autonomously. Only a required human decision, named human-controlled gate, mandatory halt or value review, external blocker, Handoff, or completion permits the turn to end. In `RELAY` operation, the Manager may end a turn while awaiting a specific User relay action, states that action explicitly, and does not claim to be polling after the turn ends.

After three stale polls or 20-30 minutes idle, the Manager intervenes by inspecting logs, recovering context, reassigning, adopting work, or completing directly when safe. The same blocker across three attempts becomes `BLOCKED` evidence and is escalated; long or high-risk Tasks have a soft 60-90 minute cap before recovery or splitting. Escalate to the User only for live-state changes, money, legal/product risk, human-controlled credentials, or accepting launch risk; code, tests, local tooling, stale serves, missing binaries, runtime flags, worktree state, Worker recovery/takeover, and report ambiguity remain Manager-owned. More than two escalated Tasks halt further dispatch until a consolidated status is produced. When a Worker stalls, disappears, or leaves work without a usable report, the Manager adopts the worktree or branch, verifies actual state against Task Log, Task Report, and Verification State where present, then continues, rolls forward, or re-dispatches based on evidence.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Overwatch Worker loop** - Worker execution follows a compact THINK / WORK / TRY rhythm: identify hazards and proof needs, make a narrow in-scope change, then prove the Task-relative behavior with commands or artifacts. Workers investigate before fixing, include real command output when a validation command can run, and continue while reversible in-scope work remains. `DONE`, `REFUTED`, `HELD`, and `BLOCKED` are proof verdict labels inside evidence only - not Task outcome statuses or lifecycle states.
<!-- OVERWATCH END -->

<!-- OVERWATCH BEGIN -->
**Plan-bound execution** - Every Task has a compact scope fence. Before editing, the Worker sends a 2-3 sentence scope echo and waits for Manager execution authorization; deviations, including unbriefed files, dependencies, infrastructure, or artifacts, require correction or Plan adjudication. Direct-agent authorization is direct; relay-mode authorizations may be batched for Tasks dispatched together.

**Plan-bound review** - Critics identify defects but cannot expand the Plan. The Planner declares each invariant's owner, fix layer, closure, and likely rejection; shared-state work cites a write/reaction-path inventory. The Manager verifies rather than inherits those claims, pins the exact validated commit, classifies escalated findings against the Task Base Commit as introduced-here or pre-existing, records one explicit disposition, and gives remediations a bounded Correction Envelope. A first pattern-class rejection inventories the Plan-defined target set and fixes all in-scope matches.

**Risk and spend controls** - Low-risk work receives one compact Manager critic pass, escalating to an independent critic if the diff adds a file, dependency, infrastructure, or artifact not named in the brief or Scope Fence, or if Manager rejection/re-review repeats. Security, privacy, schema, API, release-critical, multi-module, or demonstrated false-green risk receives two critics. Same-root recurrence and the same invariant surviving two correction envelopes are distinct hard halts; three total rejections, remediation above roughly half the original Task cost, or 60-90 minutes without convergence triggers a value review. Multi-hour continuation requires a Plan change and User approval.

**Token-efficient coordination** - Reviews stay within the pinned diff, direct dependency neighborhood, and required production-real artifact or contract references. Re-review is surgical unless the amendment expands that surface. Manager status updates occur only on dispatch, pass, rejection, halt, or escalation; waiting requires a progress signal, reason, and bounded next check. A status update never substitutes for the next coordination action or ends `DIRECT` operation.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Overwatch review validation** - The Manager treats Worker reports and summaries as hypotheses until checked against current artifacts one level closer to production when practical. Low-risk work receives one compact Manager critic pass, escalating to one independent critic when the diff adds a file, dependency, infrastructure, or artifact not named in the brief or Scope Fence, or when Manager rejection/re-review repeats. Security, privacy, schema, API, release-critical, multi-module, or demonstrated false-green risk receives Test Gate and Adversarial Change critics. Negative-control sabotage is required only when false-green risk is plausible. Every critic report pins the exact validated commit and cites evidence by path, result, and hash; a commit mismatch, amend, or rebase voids the report and requires re-validation. Review stays within the diff, its immediate dependency neighborhood, and targeted production-real artifacts or contract references; re-review is surgical unless the amendment expands that surface.

**Overwatch review adjudication** - Critic findings cannot expand the Plan. The Manager compares every escalated finding with the Task Base Commit, classifies it as introduced-here or pre-existing, records one disposition (`FIX_IN_SCOPE`, `DEFER_RESIDUAL`, `PROPOSE_PLAN_CHANGE`, `ACCEPT_RISK`, or `REJECT_FINDING`), and sends only a bounded Correction Envelope for remediation. A `MITIGATION_ONLY` Task carries a named residual and containment check, and can only receive `MERGED_WITH_RESIDUALS`. Pre-existing findings default to deferral unless they expose severe security, privacy, data-loss, financial, legal, contract, corruption, or catastrophic risk. A first pattern-class rejection inventories the Plan-defined target set and corrects all in-scope matches. The same-root and same-invariant halt sequence lives in `guides/task-review.md`; three total rejections, remediation above roughly half the original Task cost, or 60-90 minutes without convergence triggers value review. Multi-hour continuation requires a Plan change and User approval.

**Overwatch review verdicts** - `MERGE_OK`, `MERGED_WITH_RESIDUALS`, `FIX_FIRST`, `REVERT_OR_ROLL_FORWARD`, and non-counting environmental `PROOF_BLOCKED` are review evidence labels, not replacements for Task lifecycle states or Task outcome statuses. A confirmed-defect rejection records and attaches its root cause, findings, classification, disposition, invariant, and Correction Envelope to the follow-up. A distinct root cause may authorize Envelope 2, but the same invariant still violated after Envelope 2 triggers the canonical same-invariant halt; all other spend controls still apply.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Handoff verification state** - Handoff continuity includes verification state when work is in flight or recently reviewed: claims checked, verdicts issued, proof commands or artifacts, residuals still open, and any stale or rejected work that a successor must not trust blindly. Incoming agents use this state as a checklist against current artifacts, not as proof by itself.
<!-- OVERWATCH END -->
