...
<!-- OVERWATCH BEGIN -->
**Overwatch review validation** - The Manager treats Worker reports and summaries as hypotheses until checked against current artifacts. Review maps claims to diffs, files, generated bundles, command output, Task Logs, commits, runtime evidence, or Tracker state; checks freshness; and prefers evidence one level closer to production than the edited template or source. Trivial, contained work can use lightweight validation. Non-trivial work uses a two-critic gate: the Test Gate Critic checks whether proof exercises the Task trajectory and was not weakened, and the Adversarial Change Critic checks whether the change structurally closes the invariant, treats narratives as untrusted, and demonstrates the front-loaded negative control or records why it is impractical and the strongest substitute used. Review reporting states which path was used, the evidence checked, and any residual risk.

**Overwatch review verdicts** - `MERGE_OK`, `MERGED_WITH_RESIDUALS`, `FIX_FIRST`, `REVERT_OR_ROLL_FORWARD`, and non-counting environmental `PROOF_BLOCKED` are review evidence labels, not replacements for Task lifecycle states or Task outcome statuses. A confirmed-defect rejection records and attaches its root cause and critic findings to the follow-up dispatch. On a second counting rejection, the Manager halts and escalates with both summaries only if the same root cause recurred; distinct causes are recorded in Review State and corrected without a hard halt.
<!-- OVERWATCH END -->
