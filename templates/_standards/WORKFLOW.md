...
<!-- OVERWATCH BEGIN -->
Overwatch extends the Implementation Phase by tightening proof, review, continuity, and autonomous coordination around the existing Task lifecycle. It catches execution-layer failures such as stalls, weak reports, fake-green evidence, evidence ambiguity, rejection loops, and abandoned Worker output. It does not catch spec/promise drift, product gaps, requirement misreads, or planning promises that implementation quietly drops.
<!-- OVERWATCH END -->
...
**Plan-bound review** - Critics identify defects but cannot expand the Plan. Reports pin the exact validated commit and cite evidence by path, result, and hash. The Manager verifies freshness, classifies escalated findings against the Task Base Commit as introduced-here or pre-existing, records one explicit disposition, and gives remediations a bounded Correction Envelope. A first pattern-class rejection inventories the Plan-defined target set and fixes all in-scope matches.

**Risk and spend controls** - Low-risk work receives one compact Manager critic pass, escalating to an independent critic if the diff adds a file, dependency, infrastructure, or artifact not named in the brief or Scope Fence, or if Manager rejection/re-review repeats. Security, privacy, schema, API, release-critical, multi-module, or demonstrated false-green risk receives two critics. Same-root recurrence halts; three total rejections, remediation above roughly half the original Task cost, or 60-90 minutes without convergence triggers a value review. Multi-hour continuation requires a Plan change and User approval.
