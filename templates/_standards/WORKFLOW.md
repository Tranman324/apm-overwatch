...
<!-- OVERWATCH BEGIN -->
**Overwatch dispatch packet** - Task Prompts include role, work scope, invariant, non-scope, proof gate, output contract, and routing identity when those elements are needed to constrain autonomous Worker execution. For vague "all X" work, the Manager requires an inventory before dispatch so completion can be checked against a concrete set. Oversized Tasks are split when scope, risk, dependencies, or validation cannot fit a single Worker-owned proof gate.

**Autonomous coordination** - The Manager polls active Workers every 5-10 minutes, or every 2-5 minutes for tiny or near-complete Tasks. Observable progress means a Task Log update, file change, running test, Report Bus update, commit, or concrete blocker. After three stale polls or 20-30 minutes idle, the Manager intervenes by inspecting logs, recovering context, reassigning, adopting work, or completing directly when safe. The same blocker across three attempts becomes `BLOCKED` evidence and is escalated; long or high-risk Tasks have a soft 60-90 minute cap before recovery or splitting. More than two escalated Tasks halt further dispatch until a consolidated status is produced. When a stale worktree contains coherent unmerged work, the Manager inventories it, verifies it against current requirements, adopts useful changes, and discards unsafe residue.
<!-- OVERWATCH END -->
