# APM 1.0.3-overwatch.5 - Task Review Guide
...
**Proof-to-harness fit:** Require a proof method the environment can reliably and repeatably produce. Prefer headless/offscreen deterministic evidence over interactive, focus-seizing, or credential-prompting runs. When the only available proof path is unreliable, treat it as a harness gap tracked separately (see `PROOF_BLOCKED`), not repeated worker re-dispatch. Screen-seizing or credential-prompting runs require explicit User timing approval and are scheduled, not spammed.
<!-- OVERWATCH END -->
...
**Declared closure review:** Use the Plan's stable invariant ID, owning layer, fix layer, and likely rejection as review inputs, but verify them rather than inheriting them. For a shared-state Task, inspect the referenced state/reaction-path inventory against the pinned diff and immediate production-real neighborhood. `MITIGATION_ONLY` requires a named residual and a critic containment check; it cannot receive `MERGE_OK`. `UNKNOWN` requires a no-production-change spike or `PROPOSE_PLAN_CHANGE`, not another speculative fix.
...
**Contract truthing:** Before rejecting a worker for not matching an external contract (API shape, file format, OS behavior), confirm the spec matches reality via a token-safe live capture or real sample. If the spec is wrong, correct the planning document first per §3.4; do not charge the worker a rejection for conforming to a wrong spec.
...
**Correction Envelope:** For `FIX_IN_SCOPE`, record the stable invariant ID, owning layer, Plan requirement, minimum sufficient correction, inventory boundary, allowed scope, non-scope, required proof, and expansion disposition. A first pattern-class rejection requires inventory of the Plan-defined target set, correction of every in-scope match, and reporting without modification of out-of-scope matches.

**Invariant halt sequence:** Preserve one invariant ID through all correction envelopes; a root cause may change without changing the invariant. The initial confirmed-defect rejection may authorize Envelope 1. If Envelope 1 is rejected with the same root cause, issue `SAME_ROOT_HALT`: hard halt and User review asking whether the Task is too complex for one envelope or the correction is not class-level. If Envelope 1 has a distinct root cause, Envelope 2 may proceed. If Envelope 2 is rejected while the same invariant remains violated, issue `SAME_INVARIANT_HALT`: hard halt and User review asking whether the invariant is closeable at this abstraction layer. Never authorize Envelope 3 without an explicit User-approved Plan change. Record every triggered halt type, root cause, invariant, and recommendation separately.

**Remediation value gate:** Three total counting rejections, remediation above roughly 50% of the original Task's time or token cost, or 60-90 minutes of unsuccessful autonomous remediation triggers a value review: assess probability × impact, reversibility, cheap mitigation, and whether deferral is safer. Multi-hour continuation always requires an explicit Plan change and User approval. Record the selected disposition and cumulative spend in Review State.

**Review verdicts:** Record one review evidence label when helpful: `MERGE_OK` (clean acceptance), `MERGED_WITH_RESIDUALS` (proceeding with explicit residuals; required for `MITIGATION_ONLY`), `FIX_FIRST` (follow-up required before acceptance, confirmed defect), `REVERT_OR_ROLL_FORWARD` (revert or correct forward, confirmed defect), or `PROOF_BLOCKED` (proof unobtainable for environmental reasons — credential/auth prompt, harness crash, focus contention, external outage, missing timing window — with no defect shown). Distinguish "the test failed" (defect) from "the test could not run or complete" (blocked) by inspecting the actual failure signature. These labels do not replace Task lifecycle states or Task Log statuses.

**Rejected work:** A `FIX_FIRST` or `REVERT_OR_ROLL_FORWARD` finding creates a rejection summary with the pinned commit, classification, disposition, invariant ID and owning layer, root cause, rejected invariant, evidence references, and Correction Envelope. Only confirmed-defect verdicts count; `PROOF_BLOCKED` and spec-corrections do not. Track `PROOF_BLOCKED` as an environmental residual and route it to harness remediation, a scheduled timing window, or a headless path — not code re-dispatch. Apply the invariant halt sequence and value gate before another follow-up.
<!-- OVERWATCH END -->
...
<!-- OVERWATCH BEGIN -->
**Coordination mode:** At the start of continuous coordination and after Handoff or recovery, record `Coordination mode: DIRECT` in Tracker Working Notes when you can create and poll subagent Workers yourself; otherwise record `Coordination mode: RELAY`. In `DIRECT`, the autonomous standards below override the command-mediated Wait state above and any instruction to ask the User to return a Worker report. In `RELAY`, end a turn only while awaiting a specific User relay action, state that action, and do not claim active polling afterward.
...
**Direct continuity:** In `DIRECT`, do not end the current Manager turn while a Worker, reviewer, or validation command is active; a report or candidate awaits review; a Ready Task exists; or safe in-scope remediation remains. Continue local tests, builds, reviews, routine merges, and Correction Envelopes autonomously. Only a required human decision, named human-controlled gate, mandatory halt or value review, external blocker, Handoff, or completion permits the turn to end. After every Worker result, review it, act or merge or record its disposition, update the Tracker and Review State, reassess readiness, then dispatch or poll in the same turn.

**Bounded waiting and status:** Wait only with a concrete progress signal, reason, and bounded next check; otherwise continue ready work, adopt, reassign, defer, or stop. Before claiming work is active, cite a current active child handle, recent Task Log or file change, running command, Report Bus update, commit, or concrete blocker. Report Manager status only on state transitions: dispatched, passed, rejected, halted, or escalated. A status update is not a coordination action; follow it with the next bounded poll, wait, review, dispatch, or recovery action. Do not emit interim "still waiting" updates.
...
2. Investigate and determine outcome per §2.2 Review Outcome Standards:
   ...
   <!-- OVERWATCH BEGIN -->
   - Give every finding a disposition. Before escalation, classify it against the Task Base Commit. Before remediation, diagnose the root cause, apply the invariant halt sequence and value gate, and create the Correction Envelope; a pattern-class rejection also receives its Plan-defined inventory boundary.
   <!-- OVERWATCH END -->
3. Record the critic path and pinned hash, each finding's classification and disposition, invariant ID and owning/fix layers, closure, inventory reference, residual and containment result, review verdict, rejection/root-cause and invariant-envelope counts, triggered halt type, remediation spend, and any Correction Envelope in Review State when they affect future dispatch, merge, or Handoff decisions.
